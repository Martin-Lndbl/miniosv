...
{
    // A device with no MSI-X cannot be driven here at all: there is no INTx
    // path left on either architecture (see interrupt_factory). Say so rather
    // than proceed with nothing to deliver completions.
    if (!_dev->is_msix()) {
        abort("virtio: device %04x:%04x offers no MSI-X\n",
              (unsigned)_dev->get_vendor_id(), (unsigned)_dev->get_device_id());
    }
    assert(irq_factory.register_msi_bindings);
...
#ifdef __aarch64__
    // A legacy virtio device is not support on aarch64, because it needs port I/O to access its configuration registers.
    // Refuse the device rather than fault: pci-generic.cc registers it as a plain PCI device instead and the guest boots.
    size_t io_len = 0;
