...
int ena_rss_reta_update(rte_eth_dev *dev,
  ...
  for (i = 0; i < reta_size; i++) {
    ...
    if (TEST_BIT(reta_conf[conf_idx].mask, idx)) {
      ...
      if (unlikely(rc != 0)) {
        ena_log_raw(ERR, "Cannot fill indirection table");
  ...
  ena_log_raw(DBG, "RSS configured %d entries for port %d", reta_size,
              dev->data.port_id);
...
static int ena_fill_indirect_table_default(struct ena_com_dev *ena_dev,
  ...
  for (i = 0; i < tbl_size; ++i) {
    ...
    if (unlikely(rc != 0)) {
      ena_log_raw(
          DBG, "Failed to set %zu indirection table entry with val %" PRIu16 "",
          i, val);
      return rc;
    }
  }
...
static int ena_set_hash_fields(struct ena_com_dev *ena_dev, uint64_t rss_hf) {
  ...
  for (i = 0; i < ENA_ADMIN_RSS_PROTO_NUM; i++) {
    ...
    if (unlikely(rc != 0)) {
      ena_log_raw(DBG, "Failed to set ENA HF %d with fields %" PRIu16 "", i,
                  selected_fields[i].fields);
      return rc;
    }
  }
...
static int ena_rss_hash_set(struct ena_com_dev *ena_dev,
  ...
  if (rc != 0 && !(default_allowed && rc == ENA_COM_UNSUPPORTED)) {
    ena_log_raw(ERR, "Failed to set RSS hash function in the device");
    return rc;
  }
  ...
  if (rc == ENA_COM_UNSUPPORTED) {
    if (rss_conf->rss_key == NULL && !default_allowed) {
      ena_log_raw(ERR, "Setting RSS hash fields is not supported");
      return -ENOTSUP;
    }
    ena_log_raw(WARN,
...
int ena_rss_configure(struct ena_adapter *adapter) {
  ...
  if (unlikely(rc != 0 && rc != ENA_COM_UNSUPPORTED)) {
    ena_log_raw(ERR, "Failed to set indirection table in the device");
    return rc;
  }
