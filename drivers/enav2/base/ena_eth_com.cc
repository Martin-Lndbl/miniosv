...
ena_com_get_next_rx_cdesc(struct ena_com_io_cq *io_cq) {
  ...
  cdesc =
      (struct ena_eth_io_rx_cdesc_ext *)(io_cq->cdesc_addr.virt_addr +
                                         (head_masked *
...
void ena_com_dump_single_rx_cdesc(struct ena_com_io_cq *io_cq,
  ...
  if (desc) {
    uint32_t *desc_arr __attribute__((unused)) = (uint32_t *)desc;

    ena_trc_err(ena_com_io_cq_to_ena_dev(io_cq),
                "RX descriptor value[0x%08x 0x%08x 0x%08x 0x%08x] phase[%u] "
                "first[%u] last[%u] MBZ7[%u] MBZ17[%u]\n",
    ...
    if (unlikely(ena_com_is_extended_rx_cdesc(io_cq)))
      ena_trc_err(ena_com_io_cq_to_ena_dev(io_cq),
                  ...
                  (uint64_t)desc->timestamp_low | (uint64_t)desc->timestamp_high
                                                      << 32);
  }
}
  ...
  if (desc) {
    ...
    ena_trc_err(ena_com_io_cq_to_ena_dev(io_cq),
                ...
                ENA_FIELD_GET(desc->base.flags,
                              (uint32_t)ENA_ETH_IO_TX_CDESC_PHASE_MASK, 0),
                ENA_FIELD_GET(desc->base.flags,
...
ena_com_tx_cdesc_idx_to_ptr(struct ena_com_io_cq *io_cq, u16 idx) {
  idx &= (io_cq->q_depth - 1);

  return (
      struct ena_eth_io_tx_cdesc_ext *)((uintptr_t)io_cq->cdesc_addr.virt_addr +
...
/* There is an assumption in function get_tx_sq_desc() that both descriptors
 * have the same size */
static_assert(
    sizeof(struct ena_eth_io_tx_meta_desc) ==
...
ena_com_rx_cdesc_idx_to_ptr(struct ena_com_io_cq *io_cq, u16 idx) {
  idx &= (io_cq->q_depth - 1);
  return (
      struct ena_eth_io_rx_cdesc_ext *)((uintptr_t)io_cq->cdesc_addr.virt_addr +
                                        idx * io_cq->cdesc_entry_size_in_bytes);
}

static int ena_com_cdesc_rx_pkt_get(struct ena_com_io_cq *io_cq,
                                    u16 *first_cdesc_idx, u16 *num_descs) {
  ...
  do {
    ...
    if (unlikely(ENA_FIELD_GET(status, ENA_ETH_IO_RX_CDESC_BASE_FIRST_MASK,
                               ENA_ETH_IO_RX_CDESC_BASE_FIRST_SHIFT) &&
                 count != 0)) {
      ena_trc_err(dev,
    ...
    if (unlikely((status & (ENA_ETH_IO_RX_CDESC_BASE_MBZ7_MASK |
      ...
      ena_trc_err(dev, "Corrupted RX descriptor #%u on q_id: %u, req_id: %u\n",
                  count, io_cq->qid, cdesc->base.req_id);
      return ENA_COM_FAULT;
    }

    if (unlikely(cdesc->base.req_id >= io_cq->q_depth)) {
      ena_trc_err(dev, "Bad req_id in descriptor #%u on q_id: %u, req_id: %u\n",
                  count, io_cq->qid, cdesc->base.req_id);
      return ENA_COM_EIO;
    }
    ...
    count++;
    last = ENA_FIELD_GET(status, ENA_ETH_IO_RX_CDESC_BASE_LAST_MASK,
                         ENA_ETH_IO_RX_CDESC_BASE_LAST_SHIFT);
  } while (!last);
...
static int ena_com_create_meta(struct ena_com_io_sq *io_sq,
  ...
  /* bits 0-9 of the mss */
  meta_desc->word2 |=
      ENA_FIELD_PREP((u32)ena_meta->mss, ENA_ETH_IO_TX_META_DESC_MSS_LO_MASK,
                     ENA_ETH_IO_TX_META_DESC_MSS_LO_SHIFT);
  /* bits 10-13 of the mss */
  meta_desc->len_ctrl |=
      ENA_FIELD_PREP((ena_meta->mss >> 10), ENA_ETH_IO_TX_META_DESC_MSS_HI_MASK,
                     ENA_ETH_IO_TX_META_DESC_MSS_HI_SHIFT);

  /* Extended meta desc */
  meta_desc->len_ctrl |= ENA_ETH_IO_TX_META_DESC_ETH_META_TYPE_MASK;
  meta_desc->len_ctrl |=
      ENA_FIELD_PREP((u32)io_sq->phase, ENA_ETH_IO_TX_META_DESC_PHASE_MASK,
  ...
  meta_desc->word2 |= ENA_FIELD_PREP(ena_meta->l3_hdr_offset,
...
static void ena_com_rx_set_flags(struct ena_com_rx_ctx *ena_rx_ctx,
                                 struct ena_eth_io_rx_cdesc_ext *cdesc) {
  ena_rx_ctx->l3_proto =
      (ena_eth_io_l3_proto_index)(cdesc->base.status &
                                  ENA_ETH_IO_RX_CDESC_BASE_L3_PROTO_IDX_MASK);
  ena_rx_ctx->l4_proto = (ena_eth_io_l4_proto_index)ENA_FIELD_GET(
      cdesc->base.status, ENA_ETH_IO_RX_CDESC_BASE_L4_PROTO_IDX_MASK,
      ENA_ETH_IO_RX_CDESC_BASE_L4_PROTO_IDX_SHIFT);
  ena_rx_ctx->l3_csum_err = !!(ENA_FIELD_GET(
      cdesc->base.status, ENA_ETH_IO_RX_CDESC_BASE_L3_CSUM_ERR_MASK,
      ENA_ETH_IO_RX_CDESC_BASE_L3_CSUM_ERR_SHIFT));
  ena_rx_ctx->l4_csum_err = !!(ENA_FIELD_GET(
      cdesc->base.status, ENA_ETH_IO_RX_CDESC_BASE_L4_CSUM_ERR_MASK,
      ENA_ETH_IO_RX_CDESC_BASE_L4_CSUM_ERR_SHIFT));
  ena_rx_ctx->l4_csum_checked = !!(ENA_FIELD_GET(
      cdesc->base.status, ENA_ETH_IO_RX_CDESC_BASE_L4_CSUM_CHECKED_MASK,
      ENA_ETH_IO_RX_CDESC_BASE_L4_CSUM_CHECKED_SHIFT));
  ena_rx_ctx->hash = cdesc->base.hash;
  ena_rx_ctx->frag =
      ENA_FIELD_GET(cdesc->base.status, ENA_ETH_IO_RX_CDESC_BASE_IPV4_FRAG_MASK,
...
int ena_com_prepare_tx(struct ena_com_io_sq *io_sq,
  ...
  desc = (ena_eth_io_tx_desc *)get_tx_sq_desc(io_sq);
  if (unlikely(!desc))
    return ENA_COM_FAULT;

  desc->len_ctrl =
      ENA_FIELD_PREP((u32)io_sq->phase, ENA_ETH_IO_TX_DESC_PHASE_MASK,
  ...
  desc->buff_addr_hi_hdr_sz =
      ENA_FIELD_PREP((u32)header_len, ENA_ETH_IO_TX_DESC_HEADER_LENGTH_MASK,
  ...
  /* Bits 0-9 */
  desc->meta_ctrl =
      ENA_FIELD_PREP((u32)ena_tx_ctx->req_id, ENA_ETH_IO_TX_DESC_REQ_ID_LO_MASK,
                     ENA_ETH_IO_TX_DESC_REQ_ID_LO_SHIFT);

  desc->meta_ctrl |= ENA_FIELD_PREP(ena_tx_ctx->df, ENA_ETH_IO_TX_DESC_DF_MASK,
  ...
  if (ena_tx_ctx->meta_valid) {
    desc->meta_ctrl |=
        ENA_FIELD_PREP(ena_tx_ctx->tso_enable, ENA_ETH_IO_TX_DESC_TSO_EN_MASK,
...
int ena_com_rx_pkt(struct ena_com_io_cq *io_cq, struct ena_com_io_sq *io_sq,
  ...
  if (unlikely(ena_com_is_extended_rx_cdesc(io_cq)))
    ena_rx_ctx->timestamp =
        (u64)cdesc->timestamp_low | (u64)cdesc->timestamp_high << 32;
  ...
  ena_trc_dbg(ena_com_io_cq_to_ena_dev(io_cq),
              ...
              ena_rx_ctx->hash, ena_rx_ctx->frag, cdesc->base.status);
...
int ena_com_add_single_rx_desc(struct ena_com_io_sq *io_sq,
  ...
  desc = (ena_eth_io_rx_desc *)get_sq_desc_regular_queue(io_sq);
  ...
  desc->ctrl = ENA_ETH_IO_RX_DESC_FIRST_MASK | ENA_ETH_IO_RX_DESC_LAST_MASK |
               ...
               ENA_FIELD_GET(io_sq->phase, ENA_ETH_IO_RX_DESC_PHASE_MASK,
                             ENA_ZERO_SHIFT);
...
static struct ena_eth_io_tx_cdesc_ext *
ena_com_get_next_tx_cdesc(struct ena_com_io_cq *io_cq) {
  ...
  cdesc_phase = ENA_FIELD_GET(READ_ONCE8(cdesc->base.flags),
                              ENA_ETH_IO_TX_CDESC_PHASE_MASK, ENA_ZERO_SHIFT);
