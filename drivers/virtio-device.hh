...
// How a virtio driver wants its interrupts registered. The associated
// virtio-device calls this once, at bring-up.
//
// MSI-X only. The pin-based INTx fallback and the edge-triggered variants that
// used to live here went with the transports and architectures that needed
// them: aarch64 delivers PCI interrupts through the GIC ITS and deleted its
// INTx class (e7c23a86), and the MMIO transport that used the edge functors is
// not built. Every device this kernel drives -- NVMe included -- is MSI-X.
