...
#include <osv/callout.h>
#include <api/minidpdk/time.hh>

TRACEPOINT(trace_callout_init, "C=%p", void *);
TRACEPOINT(trace_callout_reset, "C=%p to_ticks=%d fn=%p arg=%p", void *,
           uint64_t, void *, void *);
...
void add_callout(callout *c) { _callouts.insert(c); }

void remove_callout(callout *c) { _callouts.erase(c); }
...
void mark_have_work(void) { _have_work = true; }

// wakes the dispatcher
void wake_dispatcher(void) { _callout_dispatcher->wake(); }
} // namespace callouts
...
int callout_reset_on(struct callout *c, u64 to_ticks, void (*fn)(void *),
  ...
  auto cur = osv::clock::uptime::now();
  int cur_ticks = ns2ticks(std::chrono::duration_cast<std::chrono::nanoseconds>(
                               cur.time_since_epoch())
                               .count());
  int result = 0;
...
int _callout_stop_safe_locked(struct callout *c, int is_drain) {
  ...
  if ((is_drain) &&
      ...
      (callout_pending(c) || (callout_active(c) && !callout_completed(c)))) {
...
void callout_init_rw(struct callout *c, rwlock *rw, int flags) {
  assert(rw != NULL);
...
void callout_init_mtx(struct callout *c, mutex *mtx, int flags) {
  assert(mtx != NULL);
...
void init_callouts(void) {
  ...
  callouts::_callout_dispatcher = sched::thread::make(
      _callout_thread, sched::thread::attr().name("callout"));
