...
// Try to absorb [fblock, fblock+count) into the last extent of a leaf, or
// append a new one. Returns true if the leaf now covers it.
bool leaf_append(fs *f, extent_header *eh, uint16_t capacity, uint32_t fblock,
                 uint64_t phys, uint32_t count)
{
    ...
    if (n > 0) {
        extent *last = &ee[n - 1];
        const uint32_t lstart = le32(last->ee_block);
        const uint16_t llen = extent_len(last);
        if (!extent_is_uninit(last) && lstart + llen == fblock &&
            extent_start(last) + llen == phys &&
            llen + count <= EXTENT_INIT_MAX_LEN) {
            last->ee_len = le16(static_cast<uint16_t>(llen + count));
            return true;
        }
    }
    ...
    set_extent(&ee[n], fblock, phys, static_cast<uint16_t>(count));
                         ...
                         uint64_t phys, uint32_t count)
{
    ...
    if (le16(eh->eh_depth) == 0) {
        if (leaf_append(f, eh, le16(eh->eh_max), fblock, phys, count)) {
            return inode_write(f, ino, in);
        }
    ...
    // depth 1: append into the last leaf, adding a leaf if it is full.
    extent_idx *ix = entries_as_idx(eh);
    uint16_t n = le16(eh->eh_entries);
    if (n == 0) {
        return -EIO;
    ...
    const uint64_t leaf = extent_idx_leaf(&ix[n - 1]);
    scratch buf(f->block_size);
    if (!buf) {
        return -ENOMEM;
    }
    ...
    if (leaf_append(f, lh, leaf_capacity(f), fblock, phys, count)) {
        return f->dev.write(buf.data(), leaf, 1);
    }

    // Leaf full: another one, if the root has room for the index.
    if (n >= le16(eh->eh_max)) {
        printf("miniext: extent tree full (%u leaves, depth 1) for inode %u -- "
    ...
    uint64_t new_leaf = 0;
    uint32_t got = 0;
    rc = block_alloc(f, phys, 1, &new_leaf, &got);
    if (rc < 0) {
        return rc;
    }

    memset(buf.data(), 0, f->block_size);
    lh = reinterpret_cast<extent_header *>(buf.data());
    lh->eh_magic = le16(EXTENT_MAGIC);
    lh->eh_entries = le16(1);
    lh->eh_max = le16(leaf_capacity(f));
    lh->eh_depth = le16(0);
    lh->eh_generation = le32(0);
    set_extent(entries_as_extents(lh), fblock, phys, static_cast<uint16_t>(count));

    rc = f->dev.write(buf.data(), new_leaf, 1);
    if (rc < 0) {
        block_free(f, new_leaf, 1);
        return rc;
    }

    ix[n].ei_block = le32(fblock);
    ix[n].ei_leaf_lo = le32(static_cast<uint32_t>(new_leaf));
    ix[n].ei_leaf_hi = le16(static_cast<uint16_t>(new_leaf >> 32));
    ix[n].ei_unused = le16(0);
    eh->eh_entries = le16(static_cast<uint16_t>(n + 1));
