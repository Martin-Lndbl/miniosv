...
`app=` names the directory to link in instead: `make app=test` for the larger test suite,
`app=app/miniduckdb` or `app=app/llama.cpp` for the submodules, or any directory holding a
Makefile that sets `$(app-objects)`.
