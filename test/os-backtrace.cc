...
constexpr auto SAMPLE_PERIOD = std::chrono::microseconds(200);
constexpr auto PHASE_TIME = std::chrono::milliseconds(1500);
...
void run_phase(const char *name, victim_mode mode, unsigned count)
{
    ...
    unsigned long faults_before = osv::unwind_faults();
    ...
    unsigned long faulted = osv::unwind_faults() - faults_before;

    printf("      %s on %u cpu%s: %lu samples (%lu walked, %lu refused, "
           "%lu truncated by a bad frame), depth %d-%d, up to %u stack "
           "switches, crossed %lu, nested %lu\n",
           mode_name(mode), count, count == 1 ? "" : "s",
           total.ticks, total.walked, total.refused, faulted,
    ...
    // A sample that came back with nothing, or with too little to be a stack,
    // is only acceptable as a walk that hit a bad frame address and gave up.
    // Anything beyond that count is the unwinder losing stacks for no reason.
    CHECK(total.empty + total.shallow <= faulted);
    // And that has to stay the rare exception, not the way backtraces behave.
    CHECK(faulted * 100 < total.ticks);
...
int os_backtrace_main()
{
    ...
    section("unwind fits on the stack it runs on in interrupt context");
    {
        ...
        CHECK(used * 2 < (size_t)CONF_interrupt_stack_size);
#else
